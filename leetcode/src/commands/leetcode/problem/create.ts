...
export default class LeetcodeProblemCreate extends Command {
  ...
  public async run(): Promise<void> {
    ...
    const code = codeSnippets.filter(c=>c.lang===language)[0].code+'\n'; // TODO get input/output and pass it to a generate test case method
...
function generateTestCode(language: string, metadata: Metadata, inputs: string[], outputs: string[]) {
  ...
  switch (language) {
    ...
    case "Python3": return (`input=[${INPUT}]\n` + // TODO python3 needs to import from typing import List and pass
      ...
      '    print(\'Passed input: \'+str(input[i]))').replace(INPUT,inputs.join(',')).replace(OUTPUT,outputs.join(',')).replace(METHOD,metadata.name);
